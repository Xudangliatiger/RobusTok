...
@torch.no_grad()
def sample_fn_with_tf(generator,
             tokenizer,
             labels=None,
             images=None,
             prob=0,
    ...
    tf_tokens = tokenizer.encode_code(images)[0].reshape(labels.shape[0], -1, 1)

    generated_tokens_tf = generator.generate(
       condition=labels,
       tf_tokens=tf_tokens,
       prob=prob,
       guidance_scale=guidance_scale,
    ...
    generated_image = tokenizer.decode_tokens(
       generated_tokens_tf.view(generated_tokens_tf.shape[0], -1), qzshape
    )    
