...
class VQModel(nn.Module):
    ...
    def encode_code(self, x, code=True):
       h = self.encode(x)
       b, c, l, _ = h.shape
       ...
       if self.product_quant > 1:
           ...
           dependency_loss = 0.0
           return self.quantize.f_to_idxBl_or_fhat(h, to_fhat=False, v_patch_nums=None)
    ...
    def decode_tokens(self, code, shape=None, channel_first=True):
        quant_b = self.quantize.get_codebook_entry(code, shape, channel_first)
        dec = self.decode(quant_b)
        return dec
