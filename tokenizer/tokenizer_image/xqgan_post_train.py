...
class PairedImageDataset(Dataset):
   """
   Wrap an existing dataset so that, in addition to (x, label),
   you also get a second image ('gt') taken from another root
   directory that mirrors the structure of the first.


   Args:
       base_ds      : Any iterable dataset that returns (x, label).
                      It must ultimately originate from an ImageFolder-style
                      dataset so that `.samples` exists somewhere down the
                      chain. If you pass a Subset(ImageFolder), it works too.
       gt_root      : Path to the second directory tree.
                      Directory layout and filenames must be identical.
       gt_transform : Optional transform applied to the paired image.
   """
   def __init__(self, base_ds, gt_root, gt_transform=None):
       self.base_ds      = base_ds
       self.gt_root      = Path(gt_root)
       self.gt_transform = gt_transform


       # Collect relative paths and labels once so __getitem__ is fast.
       self._rel_paths, self._labels = self._collect_paths_labels()


   # --------------------------------------------------------------------- #
   # Internal helpers
   # --------------------------------------------------------------------- #
   def _collect_paths_labels(self):
       """
       Walk down through possible wrappers (e.g. Subset) until we hit the
       dataset that owns `.samples`. Then build a list of relative paths
       that line up with the order of `base_ds`.
       """
       ds = self.base_ds
       subset_indices = None


       # If base_ds is a Subset, remember its indices and keep digging.
       if hasattr(ds, "indices") and hasattr(ds, "dataset"):
           subset_indices = ds.indices
           ds = ds.dataset


       # Now ds must expose .samples
       if not hasattr(ds, "samples"):
           raise AttributeError("Could not locate `.samples` in the dataset chain.")


       orig_root = Path(ds.root)


       # Either take all samples or the subset’s slice.
       if subset_indices is None:
           sampler = range(len(ds.samples))
       else:
           sampler = subset_indices
       ...
       for i in sampler:
           p, lab = ds.samples[i]
           rel_paths.append(Path(p).relative_to(orig_root))
           labels.append(lab)
   ...
   def __getitem__(self, idx):
       # x and label come straight from the original dataset (already transformed)
       x, label = self.base_ds[idx]


       # Locate the paired image and apply its transform (if any)
       gt_path = self.gt_root / self._rel_paths[idx].with_suffix(".JPEG")


       if not gt_path.is_file():
           raise FileNotFoundError(f"Paired image not found: {gt_path}")


       gt_img = Image.open(gt_path).convert("RGB")
       gt     = self.gt_transform(gt_img) if self.gt_transform else gt_img
...
def parse_args():
   ...
   parser.add_argument("--pair-set", type=str, default='path/to/imagenet')
...
def main(args):
   ...
   dataset = PairedImageDataset(dataset, gt_root=args.pair_set, gt_transform=gt_transform)
   ...
   # Setup optimizer
   trainable_param = (p for p in vq_model.parameters() if p.requires_grad)
   optimizer = torch.optim.AdamW(trainable_param, lr=args.lr, betas=(args.beta1, args.beta2),
   ...
   for epoch in range(start_epoch, args.epochs):
       ...
       for x, y, gts in loader:
           ...
           with torch.cuda.amp.autocast(dtype=ptdtype): 
               ...
               loss_gen = vq_loss(codebook_loss, sem_loss, detail_loss, dependency_loss, gts, recons_imgs, optimizer_idx=0, global_step=train_steps+1,
           ...
           with torch.cuda.amp.autocast(dtype=ptdtype):
               loss_disc = vq_loss(codebook_loss, sem_loss, detail_loss, dependency_loss, gts, recons_imgs, optimizer_idx=1, global_step=train_steps+1,
                                   logger=logger, log_every=args.log_every, fade_blur_schedule=fade_blur_schedule)
           ...
           if train_steps % args.log_every == 0:
               ...
               if dist.get_rank() == 0:
                   ...
                   if train_steps % args.vis_every == 0:
                       ...
                       image = torch.cat(recons_with_scale + [imgs[:4]] + [gts[:4]], dim=0)
